...
# 3. Per-repo init — creates `.df/` at the git root
npm run df -- init
```
...
npm run df -- tail   <run-id> --follow   # human-readable activity stream
npm run df -- logs   <run-id> --follow   # raw events
npm run df -- kill   <run-id>

# Cross-run memory
npm run df -- history                    # print .df/journal.md
npm run df -- findings                   # recurring findings across runs
npm run df -- findings suppress <fp>     # silence a known false positive
npm run df -- onboard                    # profile the repo for future runs

# Long-running worker that picks up queued runs
npm run worker
```
...
`.df/runs/<id>/.claude/settings.json` with a `PreToolUse` hook pointing at
`guard.ts`. The guard:

- rejects any tool call if `.df/runs/<id>/KILLED` exists (`df kill` path)
...
| tests | Repo's real test command from `.df/profile.json` — non-zero exit = HIGH | Subprocess (no LLM) |
...
default; set `DF_ADVERSARIAL_REVIEW=off` to disable, or `=on` to require it
(stage fails if unavailable).

## Cross-run memory

Three files under `.df/` feed future runs by being auto-injected into spec
and design prompts:

- **`.df/journal.md`** — one stanza per completed run. Activity log: what was
  asked, what shipped, cost. Written by the deliver stage.
- **`.df/decisions.md`** — ADR-lite entries for non-obvious design
...
- **`.df/profile.json`** — one-shot repo profile (language, test command,
  conventions). Written by `df onboard`, refreshed with `df onboard --refresh`.

Plus the **findings ledger** — aggregated across runs from the `findings`
SQLite table. Shown by `df findings`, and the top recurring entries are
...
|---|---|---|
| journal.md | What did df do last time? | Every completed run |
...
| profile.json | What is this repo? (language, test cmd) | `df onboard` |

## Layout

```
src/
├── cli.ts                          # df init|new|run|status|logs|tail|kill
...
.df/                                # per-project state (gitignored)
├── project.json                    # project marker (written by df init)
├── dark-factory.db                 # SQLite: runs, stages, findings, sessions
├── journal.md | decisions.md       # cross-run memory
├── profile.json                    # repo profile (df onboard)
└── runs/<id>/                      # per-run artifacts
    ├── .claude/settings.json       # sandbox hook config
    ├── KILLED                      # (sentinel; present if `df kill`ed)
...
| `DF_TIMEOUT_SEC_PER_RUN` | 3600 | Wall-clock cap per run |
| `DF_TIMEOUT_SEC_PER_STAGE` | 600 | Wall-clock cap per stage |
| `DF_MAX_REVIEW_ITERS` | 3 | implement ⇄ review loop cap |
| `DF_MAX_CONCURRENT_RUNS` | 2 | Worker concurrency |
| `DF_MODEL` | (claude default) | Pass to every `claude` invocation |
| `DF_ROOT` | cwd | Project root (for worker processes running elsewhere) |
| `DF_ADVERSARIAL_REVIEW` | auto | `auto` \| `on` \| `off` — optional Codex critic |
| `DF_TESTS_CRITIC` | auto | `auto` \| `on` \| `off` — mechanical test critic |
| `DF_SPEC2TESTS` | auto | `auto` \| `on` \| `off` — generate test scaffolds from spec |
| `DF_CODEX_COMPANION` | (auto-discover) | Absolute path to `codex-companion.mjs` |
